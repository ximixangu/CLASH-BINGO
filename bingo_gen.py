...
def create_cell_content(cell_type):
    ...
    if cell_type == 'triplet':
        ...
        if card:
            load_and_paste(img, CARDS_PATH / card, (180, 180), (8, 0))
        load_and_paste(img, ASSETS_PATH / "aux/boost2.png", (75, 75), (114, 115))
        # if card:
        #     load_and_paste(img, CARDS_PATH / card, (180, 180), (8, 0))
...
if __name__ == "__main__":
    ...
    with st.sidebar.expander("Parameters", expanded=False):
        ...
        with col2:
            ...
            arena_rate = st.slider("Arena weight", 0.01, 10.0, 0.1)
            misc_rate = st.slider("Misc. weight", 0.01, 10.0, 1.0)
            elixir_rate = st.slider("Elixir weight", 0.01, 10.0, 0.1)
    ...
    "Modifiers should override contitions if necessary. (e.g. All commons + Champion must be 7 commons and 1 champion.)")